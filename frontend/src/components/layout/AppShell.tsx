  ...
  return (
    <div className={`app-shell ${theme}`}>
      ...
      <aside className={`app-sidebar ${drawerOpen ? "drawer-visible" : ""}`} style={{ width }}>
        ...
        <Link href="/create" className="new-document" onClick={() => setDrawerOpen(false)}>＋ New Document</Link>
        ...
        <div className="sidebar-footer"><Link href="/settings">⚙ Settings</Link><Link href="/help">? Help &amp; Feedback</Link><button className="theme-button" onClick={() => setTheme(theme === "light" ? "dark" : "light")}>{theme === "light" ? "☼ Light mode" : "◐ Dark mode"}<span className={`switch ${theme === "dark" ? "on" : ""}`}><i /></span></button><div className="profile"><span className="avatar">JD</span><span><b>Workspace User</b><small>workspace@docuai.local</small></span></div></div>
        <button className="resize-handle" aria-label="Resize sidebar" onPointerDown={() => { resizing.current = true; }} />
      </aside>
      <main className="app-main"><header className="app-header"><button className="mobile-menu" aria-label="Open navigation" onClick={() => setDrawerOpen(true)}>☰</button><div className="mobile-brand"><span className="brand-mark">D</span> DocuAI</div><div className="crumb">Workspace <span>/</span> <b>{title}</b></div><div className="header-tools"><button aria-label="Toggle theme" onClick={() => setTheme(theme === "light" ? "dark" : "light")}>{theme === "light" ? "☼" : "◐"}</button><span className="avatar mini">JD</span></div></header><div className="app-content">{children}</div></main>
    </div>
  );
}
